...
class DeepConv_Generator(pl.LightningModule):
    def __init__(
        ...
        self.generator = nn.Sequential(
            deepconv_generator_block(
                input_dim, hidden_dim * 4, stride=stride[0], kernel_size=kernel[0]
            ),
            deepconv_generator_block(
                hidden_dim * 4, hidden_dim * 8, stride=stride[1], kernel_size=kernel[1]
            ),
            deepconv_generator_block(
                hidden_dim * 8, hidden_dim * 4, stride=stride[2], kernel_size=kernel[2]
            ),
            deepconv_generator_block(
                hidden_dim * 4, hidden_dim * 2, stride=stride[3], kernel_size=kernel[3]
            ),
            deepconv_generator_block(
                hidden_dim * 2, hidden_dim, stride=stride[4], kernel_size=kernel[4]
            ),
        )
