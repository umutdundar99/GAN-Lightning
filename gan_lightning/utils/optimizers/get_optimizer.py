...
def get_optimizer(model_params, optimizer_dict: str, **kwargs):
    ...
    if optimizer_dict["optimizer_name"] not in [optimizer.__name__.lower() for optimizer in __optimizers__]:
    ...
    for optimizer in __optimizers__:
        if optimizer.__name__.lower() == optimizer_dict["optimizer_name"].lower():
            optimizer = optimizer
            break
    logging.info("Using optimizer: {}".format(optimizer_dict["optimizer_name"].capitalize()))

    if scheduler_dict is None:
        logging.info("No scheduler found")
        return optimizer(lr=optimizer_dict["lr"], **kwargs)
    else:
        if scheduler_dict["scheduler_name"] not in [scheduler.__name__ for scheduler in __schedulers__]:
            raise ValueError("Scheduler {} not found".format(scheduler_dict["scheduler_name"]))
        ...
        return optimizer, scheduler(optimizer, step_size=scheduler_dict["step_size"], gamma=scheduler_dict["gamma"])
