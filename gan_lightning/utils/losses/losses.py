...
class WDiscLoss(torch.nn.Module):
    ...
    def forward(self, d_real_prediction, d_fake_prediction, penalty, penalty_lambda: float = 10.0):
        return torch.mean(d_fake_prediction) - torch.mean(d_real_prediction) + penalty_lambda * penalty
